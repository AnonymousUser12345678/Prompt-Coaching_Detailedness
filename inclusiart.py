...
user_data_collection = db['detailed_data']  # Collection name
...
def get_undetailed_example(user_prompt):
    response = client.chat.completions.create(
        model="gpt-4o-mini",
        messages=[
            {"role": "system", "content": """
            You are an expert in analyzing character descriptions for storytelling and visual representation. Focus on identifying areas where the description lacks sufficient detail, such as:
            - Vague or generic physical descriptions
            - Lack of personality traits or motivations
            - Missing background or context
            - Absence of unique characteristics or quirks
            - Insufficient information about the character's role or purpose
            Provide specific, concrete examples of where more detail could enhance the character description.
            """},
            {"role": "user", "content": f"Analyze this character description: '{user_prompt}'. In one clear, specific sentence, identify the most significant area where the description lacks detail for a comprehensive visual or narrative representation."}
        ],
        max_tokens=100,
        temperature=0.7
    )
...
def suggest_detailed_prompt(user_prompt, undetailed_example):
    response = client.chat.completions.create(
        model="gpt-4-turbo",
        messages=[
            {"role": "system", "content": """You are an expert in enhancing character descriptions with specific details. Your task is to:
- Maintain the core concept of the original character
- Expand slightly on personality traits, background, and unique characteristics
Offer only the enhanced description without any explanation.
IMPORTANT: The detailed suggestion should have the same wordcount as the original description
             """},
            {"role": "user", "content": f"Enhance this character description with more vivid, specific details while keeping the same core concept: '{user_prompt}'. Address this lack of detail: '{undetailed_example}'. Do not include any introductory text before the description. IMPORTANT: Make sure the enhanced description has the same wordcount as the original description."}
        ],
        max_tokens=200,
        temperature=0.8
    )
...
if 'detailed_suggestion' not in st.session_state:
    st.session_state['detailed_suggestion'] = ""
if 'random_object' not in st.session_state:
    st.session_state['random_object'] = ""
if 'undetailed_example' not in st.session_state:
    st.session_state['undetailed_example'] = ""
...
if st.session_state['prolific_id_submitted']:
    ...
    if st.session_state['user_prompt_submitted']:
        ...
        st.warning("""
        AI models may produce surprising or random outputs, especially when prompts lack detail.
        """)

        # Step 5: Generate lack of detail example and detailed suggestion
        if not st.session_state['detailed_suggestion']:
            with st.spinner('Analyzing your prompt for potential lack of detail...'):
                st.session_state['undetailed_example'] = get_undetailed_example(st.session_state['user_prompt'])
            with st.spinner('Suggesting more detailed alternatives...'):
                if st.session_state['undetailed_example']:
                    st.session_state['detailed_suggestion'] = suggest_detailed_prompt(st.session_state['user_prompt'],st.session_state['undetailed_example'])

        if st.session_state['undetailed_example']:
            st.write(f"Example of potential lack of detail: {st.session_state['undetailed_example']}")

        # Step 6: Display suggested detailed prompt
        st.write("Suggested Detailed Prompt:")
        st.code(st.session_state['detailed_suggestion'])
        ...
        if not st.session_state['final_confirmed']:
            ...
            if st.button("Confirm Final Prompt"):
                if final_prompt.strip() == "":
                    final_prompt = st.session_state['detailed_suggestion']
        ...
        if st.session_state['final_confirmed']:
            ...
            if st.session_state["feedback_given"]:
                ...
                if not st.session_state['additional_feedback_submitted']:
                    additional_feedback = st.text_input("Any feedback about the details in the AI-generated image?", key="additional_feedback_input")
                    if additional_feedback:
                        st.session_state['additional_feedback'] = additional_feedback
                        st.session_state['additional_feedback_submitted'] = True
                        st.rerun()
                else:
                    st.text_input("Any feedback about the details in the AI-generated image?", value=st.session_state['additional_feedback'], disabled=True)

                if st.session_state['additional_feedback_submitted']:
                    # Step 10: Test user's understanding of detailed prompting
                    if not st.session_state['random_object']:
                        st.session_state['random_object'] = get_random_object("")

                    st.write(f"Now, to test your understanding of detailed prompting, write an detailed prompt to generate an image of: **{st.session_state['random_object']}**")
                            ...
                            if not st.session_state['save_button_clicked']:
                                ...
                                if st.button("Save and Receive Code"):
                                    ...
                                    data = {
                                        ...
                                        'undetailed_example': st.session_state.get('undetailed_example', ''),
                                        'detailed_suggestion': st.session_state.get('detailed_suggestion', ''),
